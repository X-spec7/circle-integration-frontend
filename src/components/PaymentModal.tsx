...
const PaymentModal: React.FC<PaymentModalProps> = ({ project, isOpen, onClose }) => {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-2xl max-w-md w-full p-6">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-bold text-gray-900">Make Investment</h2>
          <button
            onClick={onClose}
            ...
            <div className="p-4 bg-gray-50 rounded-lg">
              <div className="flex justify-between items-center">
                ...
                <span className="text-lg font-bold text-gray-900">
                  {calculateTokens().toLocaleString()} {project.symbol}
                </span>
              </div>
          ...
          <div className="p-4 bg-blue-50 rounded-lg">
            <div className="flex items-start">
              {paymentMethod === 'fiat' ? (
                ...
                <Coins className="h-5 w-5 text-blue-600 mr-2 mt-0.5" />
              )}
              <div>
                <h4 className="text-sm font-medium text-blue-900">
                  {paymentMethod === 'fiat' ? 'SEPA Bank Transfer' : 'Crypto Payment'}
                </h4>
                <p className="text-sm text-blue-700 mt-1">
                  {paymentMethod === 'fiat' 
                    ? 'You will receive bank details to complete the transfer.' 
                    : 'You will receive the escrow address to send USDC.'
                  }
                </p>
              </div>
            </div>
